...
function render(next) {
  ...
  $('#plan-pnl').textContent = `${next.pulse.planPnl >= 0 ? '+' : '-'}$${Math.abs(next.pulse.planPnl).toFixed(2)}`;
...
function renderPositions(positions) {
  $('#positions').innerHTML = positions.length ? positions.slice(0, 5).map(p => `<div class="position"><strong>$${p.token}</strong><span>$${fmt(p.size)}</span><span class="${p.pnl >= 0 ? 'profit' : 'loss'}">${p.pnl >= 0 ? '+' : '-'}$${Math.abs(p.pnl).toFixed(2)}</span></div>`).join('') : `<p>${state.config.enabled ? 'Armed. Waiting for threshold.' : 'No planned entries. Autosnipe is disarmed.'}</p>`;
}
