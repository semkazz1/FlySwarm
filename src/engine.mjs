...
export class FlySwarmEngine {
  ...
  detect(token, memory) {
    ...
    const signal = {
      ...
      evidence: [
        ...
        `$${token.liquidity.toLocaleString('en-US')} liquidity input`
      ]
    };
  ...
  snapshot() {
    ...
    return {
      meta: {
        mode: 'ALPHA', seed: this.seed, tick: this.tickNo,
        disclaimer: 'Prototype cohort metrics. Read-only execution boundary.'
      },
      pulse: {
        ...
        neuronMotifs: 14,
        planPnl: Math.round(this.positions.reduce((sum, p) => sum + p.pnl, 0) * 100) / 100
      },
      config: { ...this.config },
