...
export class RobinhoodLive {
  ...
  async holders(address) {
    ...
    catch (error) { return this.fallbackHolders(address, error.message); }
  }
  ...
  fallbackHolders(address, reason) {
    ...
    return {
      mode: 'PROTOTYPE',
      source: 'Prototype holder map',
      coverage: 'fallback holder map',
      heuristic: `RPC unavailable (${reason}). Prototype holder map active; token selection remains live`,
      token: { address }, items
    };
  }
}
