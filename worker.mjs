...
assert.equal(state.meta.mode, 'ALPHA');
...
assert.ok(state.positions.length > 0, 'expected a planned position');
assert.ok(state.positions.every(p => p.size === 150), 'expected configured position size');
console.log(`ok · ${state.transfers.length} transfers · ${state.signals.length} signals · ${state.positions.length} planned positions`);
