# Methodology and honest boundary
...
The source connectome contains tens of millions of synapses. FlySwarm is a product simulation, not neuroscience analysis, so bundling that dataset would make the repository much larger without validating the market hypothesis. No biological data file is downloaded during install or runtime.

## The compact motif

The simulation borrows only a general graph idea:
...
evidence score (decision)
        ↓
paper position (simulated action)
```
...
- 18% synthetic liquidity sufficiency
...
Hunter aliases, profitable-wallet status, inferred funding paths, cohort memory, hit rates, signal scores, returns and paper positions are invented. The interface always displays `SIMULATION` around those features, and the server binds only to `127.0.0.1` by default. There is no wallet connection, private-key input, signer or transaction broadcaster.
